...
namespace RBush
{
	public partial class RBush<T>
	{
		...
		/// <summary>
		/// return all box that overlap to the given box
		/// </summary>
		/// <param name="boundingBox"></param>
		/// <returns></returns>
		private List<ImmutableStack<ISpatialData>> DoOverlap(in Envelope boundingBox)
		{
			var node = this.root;
			if (!node.Envelope.Intersects(boundingBox))
				return new List<ImmutableStack<ISpatialData>>();

			var insider = new List<ImmutableStack<ISpatialData>>();
			var queue = new Queue<ImmutableStack<ISpatialData>>();
			queue.Enqueue(ImmutableStack<ISpatialData>.Empty.Push(node));
			...
			do
			{
				...
				foreach (var c in (current.Peek() as Node).Children)
				{
					if(boundingBox.Contains(c.Envelope))
					{
						if (c is T)
							insider.Add(current.Push(c));
						else
							queue.Enqueue(current.Push(c));
					}
				}
